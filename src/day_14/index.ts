...
export function process(data: string[], hasFloor = false) {
    ...
    let rocks = data.map(line => {
        ...
        do {
            match = re.exec(line)
            if (match !== null) {
                coords.push({ x: Number.parseInt(match[1]), y: Number.parseInt(match[2]) })
            }
        } while (match != null)
        return coords
    })
    ...
    const maxX = Math.max(...flatPoints.map((coord) => coord.x)) - preshiftMinX + 1
    // Find maxY
    const maxY = Math.max(...flatPoints.map((coord) => coord.y))

    // Move X origin to minX (new origin at X:1,Y:0)
    rocks = rocks.map(line => line.map(coord => ({ y: coord.y, x: coord.x - preshiftMinX })))
    ...
    do {
        ...
        if (currentGrain.x === 0) {
            grid.forEach(line => { line.unshift(null) })
            SAND_ORIGIN = { y: 0, x: SAND_ORIGIN.x + 1 }
        } else if (currentGrain.x + 1 === grid[0].length) {
            // shift left
            grid.forEach(line => { line.push(null) })
        }

        // eslint-disable-next-line no-unmodified-loop-condition
    } while (!coordEq(currentGrain, SAND_ORIGIN) && (hasFloor || !isIntoAbyss(currentGrain, maxY)))
...
function drawGrid(rocks: Coord[][], maxX: number, maxY: number): Grid {
    ...
    const grid: Grid = Array.from({ length: maxY + 2 }, () => Array.from({ length: maxX + 1 }, () => null));
...
function getNextMove(grid: Grid, currentCoord: Coord, floorY: number | null): Coord | null {
    // detect floor
